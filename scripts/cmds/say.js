...
// ─── Edge TTS via HTTP (no WebSocket dependency) ──────────────────────────
...
async function edgeTTSHttp(text, voice, style) {
...
// ─── Edge TTS via WebSocket (fallback) ────────────────────────────────────
async function edgeTTSWs(text, voice, style) {
  let WebSocket;
  try { WebSocket = require("ws"); } catch (_) { throw new Error("ws not available"); }
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      if (!done) { done = true; try { ws.terminate(); } catch(_) {} reject(new Error("Edge TTS WS timeout")); }
    }, 20000);

    const langTag = voice.slice(0, 5);
    let ssml;
    if (style) {
      ssml = `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='https://www.w3.org/2001/mstts' xml:lang='${langTag}'><voice name='${voice}'><mstts:express-as style='${style}'>${escapeXML(text)}</mstts:express-as></voice></speak>`;
    } else {
      ssml = `<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='${langTag}'><voice name='${voice}'>${escapeXML(text)}</voice></speak>`;
    }
    ...
    ws.on("message", (data) => {
      if (Buffer.isBuffer(data)) {
        ...
        done = true; clearTimeout(timeout); try { ws.close(); } catch(_) {}
        resolve(Buffer.concat(chunks));
      }
    });
    ws.on("error", (err) => { if (!done) { done = true; clearTimeout(timeout); reject(err); } });
    ws.on("close", () => { if (!done && chunks.length > 0) { done = true; clearTimeout(timeout); resolve(Buffer.concat(chunks)); } else if (!done) { done = true; clearTimeout(timeout); reject(new Error("WS closed with no data")); } });
  });
}

// ─── Edge TTS: try HTTP first, then WS ───────────────────────────────────
async function edgeTTS(text, voice, style) {
  try {
    return await edgeTTSHttp(text, voice, style);
  } catch (_) {
    return await edgeTTSWs(text, voice, style);
  }
}
...
// ─── Alternative Google TTS (backup) ──────────────────────────────────────
async function googleTTS2(text, lang) {
  const truncated = text.slice(0, 200);
  const url = `https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(truncated)}&tl=${lang}&client=gtx&tk=0`;
  const res = await axios.get(url, {
    responseType: "arraybuffer", timeout: 20000,
...
module.exports = {
  ...
  onStart: async function ({ api, event, args, threadsData }) {
    ...
    if (args[0]?.toLowerCase() === "voice") {
      ...
      return api.sendMessage(
        `✅ تم تغيير الصوت!\n\n${v.label}\n${v.desc}\n\n` +
        `${v.engine === "edge" ? "🤖 Microsoft AI — يدعم 40+ لغة!" : "✅ يدعم 40+ لغة."}`,
        threadID, messageID
      );
    }
    ...
    if (!args[0]) {
      ...
      if (LANG_CODES[firstWord]) {
        ...
        if (!text) return api.sendMessage(`❌ أضف نصاً بعد كود اللغة.\nمثال: /say ${forceLang} مرحبا`, threadID, messageID);
      } else {
        text = args.join(" ").trim();
      }
    }
    ...
    // Use /tmp for cache — always writable and auto-cleaned
    const cacheDir = "/tmp/say_cache";
    await fs.ensureDir(cacheDir);
    const filePath = path.join(cacheDir, `say_${senderID}_${Date.now()}.mp3`);

    const cleanup = () => fs.remove(filePath).catch(() => {});
    ...
    try {
      ...
      if (voiceConfig.engine === "google" || GOOGLE_FALLBACK.has(lang)) {
        ...
        try {
          ...
          audioBuffer = await googleTTS(text, lang);
          infoLine = `🌐 Normal (fallback) | 🌍 ${langName}`;
        }
      }
      ...
      // Last resort: try backup Google TTS method
      try {
        ...
        const audio = await googleTTS2(text, validLang);
        await fs.writeFile(fallbackPath, audio);
        return await api.sendMessage(
          { body: `🔊 🌐 ${LANG_CODES[validLang] || validLang}`, attachment: fs.createReadStream(fallbackPath) },
          threadID,
      ...
      return api.sendMessage(
        "❌ فشل توليد الصوت. حاول مجدداً أو استخدم /say voice normal",
        threadID, messageID
      );
    }
  }
